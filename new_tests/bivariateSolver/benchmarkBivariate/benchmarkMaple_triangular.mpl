...
writeFile:=fopen("results_maple_triangular.txt",WRITE);
...
fprintf(writeFile, "--------------------------------------------\n");
fprintf(writeFile, "a \t b \t #Sol \t elapsed \t algorithm \n");
fprintf(writeFile, "--------------------------------------------\n");
...
for i from 1 to nElements do
    ...
    nSolutions:=map(RegularChains:-ChainTools:-NumberOfSolutions, dec, R):
    fprintf(writeFile,"%d \t %d \t %d \t %f \t(Triangularize) \n" ,a[i], b[i], nSolutions[1], elapsedTime):
    print("Triangularize", a[i], b[i], nSolutions[1], elapsedTime):
