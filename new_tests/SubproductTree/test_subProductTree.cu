...
//	sfixn *T = new sfixn[length * no];
	sfixn *T = (sfixn*) malloc (sizeof(sfixn) * (length * no));
	for (j = k; j >= 1; --j)
	{
...
main (int argc, char *argv[])
{
	...
	if (verify == 1)
	{
		...
		/*
		 ...
		 v)   Fast evaluation
		 */
		i = system ("maple -q maple_subProductTree.mm");
	}
//	sfixn *points = new sfixn[numPoints];
//	sfixn *F = new sfixn[numPoints];
	sfixn *points = (sfixn*) malloc (sizeof(sfixn) * numPoints);
	sfixn *F = (sfixn*) malloc (sizeof(sfixn) * numPoints);
	...
	struct PolyEvalSteps check = onlySubproductTree (F, points, k, p, verify);
	...
	if (verify == 1)
	{
		...
		// cout<<"Testing the outcome of subproduct tree with Maple for every steps. with k = "<<k<<endl;
		printf (
				"Testing the outcome of subproduct tree with Maple for every steps. with k = %d\n",
				k);

		int testI = system ("diff -b PolyM.dat PolyMgpu.dat");
		if (testI != 0)
			// cout<<"  Error in subproduct tree"<<endl;
			printf ("Error in subproduct tree\n");
		else
			// cout<<"PASS"<<endl;
			printf ("PASS\n");
	}
	...
	if (k > plainMulLimit)
	{
		cudaFree (check.InvMl[plainMulLimit]);
		cudaFree (check.InvMr[plainMulLimit]);
	}
