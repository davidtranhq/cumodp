...
{
  __shared__ int xsm[TN*BC];  
  __shared__ int ysm[TN*BC]; 
  __shared__ int usm[TN*BC]; 
  int tid = blockIdx.x*blockDim.x + threadIdx.x;   
  short i, pos, t;
  int *xd, *yd, *ud, *xm, *ym, *um; 
...
int main(int argc, char *argv[])
{
  ...
  printf("we have done 100 times of us = (xs * ys) mod 469762049 with shared memory, %d normal number pair.\n", BN*TN*BC);
