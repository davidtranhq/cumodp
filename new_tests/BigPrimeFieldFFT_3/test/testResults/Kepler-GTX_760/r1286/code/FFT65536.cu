...
__device__ void mulOmega(unsigned long *xs, int pid, unsigned long *ws)
{
  ...
  rp=pid>>12; //w^65536=r  0<=rp<16  caution: depend on N.
  wp=pid-(rp<<12); //wp<65536
...
void FFT65536(unsigned long *xs, unsigned long *ws)
{
  int k=4;
  ...
  printf("we have done xs = FFT65536(xs) in 1 times.\n");
...
int main(int argc, char *argv[])
{
  ...
  sprintf(fileName, "FFT_65536_input.dat"); 
  ...
  memset(fileName, 0 , sizeof(char)*1024);
  sprintf(fileName, "FFT_65536_output.dat"); //INC*8 (unsigned long) data
