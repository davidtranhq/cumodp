...
__device__ void mulOmega(unsigned long *xs, int pid, unsigned long *ws)
{
  ...
  rp=pid>>8; //w^256=r  0<=rp<16  caution: depend on N.
  wp=pid-(rp<<8); //wp<256
...
void FFT4096(unsigned long *xs, unsigned long *ws)
{
  int k=3;
  ...
  printf("we have done xs = FFT4096(xs) in 1 times.\n");
...
int main(int argc, char *argv[])
{
  ...
  sprintf(fileName, "FFT_4096_input.dat"); 
  ...
  memset(fileName, 0 , sizeof(char)*1024);
  sprintf(fileName, "FFT_4096_output.dat"); //INC*8 (unsigned long) data
