...
writeFile:=fopen("../../test/tmp/uData_maple_6",WRITE,TEXT):
# writeFile:=fopen("uData_maple_6",WRITE,TEXT):
...
for i from 1 to vecSize do
  ...
  # xs:=xData[(i-1)*8+1..i*8]:
  # xs:=xs[1..8]:
  xN[i]:=toBigInt(xs,r,p):
  # print(xN  [i]);
end do:

# compute FFT4096 over vector of big integers, of size 4096
