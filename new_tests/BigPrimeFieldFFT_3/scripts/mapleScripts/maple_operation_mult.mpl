...
writeFile:=fopen("../../test/tmp/uData_maple_1",WRITE):
...
#computing bigMultiplication using Maple arithmetic
bigMultMaple:=proc(xs,ys,r,p)
  #convert xs and ys to bigInt xN and yN, respectively
  xN:=toBigInt(xs,r,p):
  yN:=toBigInt(ys,r,p):

  #multiplication modulo big prime "p"
  result:=(xN*yN) mod p:
  #convert result of multiplication to vector of k=8 coefficients
  resultVector:=toBigFieldElement(result,r,p,8):
  #write back result of multiplication to "uData_mult_maple"
