...
nvcc -DLINUXINTEL64 --cudart shared --compiler-options '-fPIC -Wall -Wno-unused-but-set-variable -Wno-comment' $GENCODE_ARCH_COMPUTE -w -O2 -Xptxas -dlcm=cg $PROJECT/src/bigArithmetic_test.cu -I $CUMODP_HOME/include -L $CUMODP_HOME/src -lcumodp -o $PROJECT/bin/bigArithmetic_test.bin
