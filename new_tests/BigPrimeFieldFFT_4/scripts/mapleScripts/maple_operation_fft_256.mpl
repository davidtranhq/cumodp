...
writeFile:=fopen("../../test/tmp/uData_maple_5",WRITE, TEXT):
...
# writeFile:=fopen("/home/dave/Desktop/DESK_SEPTEMBER/FFT/repository/cumodp/new_tests/BigPrimeFieldFFT_3/test/data/data_2_11_22_1k/uData_maple_fft256",READ,TEXT):

# writeFile:=fopen("uData_maple_fft256",WRITE,TEXT):
...
FFT256:=proc(a,N,w,p)
  ...
  for n from 1 to N/256 do
  offset:=(n-1)*256:
  print(offset);
    for j from 1 to J do
      for k from 1 to K do
        # rs[j,k]:=a[(k-1)*J+j]:
        rs[j,k]:=a[offset+(k-1)*J+j]:
      end do:
    end do:
...
# result:=FFT256(xN, 256, omega, p):
result:=FFT256(xN, vecSize, omega, p):
