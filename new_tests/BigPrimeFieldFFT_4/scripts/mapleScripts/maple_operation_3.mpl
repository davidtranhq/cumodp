...
writeFile:=fopen("../../test/tmp/uData_maple_3",WRITE):
...
#computing shifted vector using Maple arithmetic
bigShiftMaple:=proc(xs, nShift, r, p)
  local xN, i, yN;
  #convert xs and ys to bigInt xN and yN, respectively
  # xN:=toBigInt(xs,r,p):
  # xN:=convert(xs,Array):
  # tmp:=Array(1..k,i->0):
  # j:=k-nShift:
  # for i from 1 to nShift do
  #   tmp[i]:=xN[j]:
  #   j:=j+1:
  # end do:

  # for i from k by -1 to nShift+1 do
  #   xN[i]:=xN[i-nShift]:
  # end do:
  ...
  # # xN:=(xN-tmp) mod p:
  # yN:=bigSubMaple(xN, tmp, r, p):
  xN:=toBigInt(xs,r,p);
  xN:=modp(xN*r^nShift,p);
  yN:=toBigFieldElement(xN,r,p,k);
  resultArray:=convert(yN,array):
