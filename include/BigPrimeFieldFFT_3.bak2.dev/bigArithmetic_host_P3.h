...
////	}
////	e=4;
//	printf ("in general FFT for K^e for K=16 and e=%d\n", e);
//	short m = e;
//	short j = 0;
//	if (e % 2 == 1)
//	{
...
//	}
//
//	//do fft-even on k^(e-1) points
//	/*
//	 * for (i=m;i>2;i-=2)
//	 * 		permutate_256(k^2,k^i)//every 256^i = k^(2i) elements
//	 * 		host_permutation_256_general_permutated(fd, i-1);//permutation L(2^(i-1),256);
//	 *
//	 */
//	host_init_multiplication_permutated (fd);
...
//	{
////		permutate_256(k^2,k^i)//every 256^i = k^(2i) elements
//		for (j = 0; j < (i) / 2; j++) //L(k^2,K^i)=i/2 L(k^i,k^2)
//			host_permutation_256_general_permutated (fd, (i) * 4); //permutation L(K^(i),256)=L[2^(4*i),256];
//	}
//
//	/*
//	 * for (i=2;i<=m;i+=2)
//	 * 		fft_256 on whole input vector
//	 * 		twiddle_general (i,k^2)
//	 * 		permutate_256(k^i,k^2)//every 256^i = k^(2i) elements
//	 * 		fft_256 on whole input vector
//	 * 		permutate_256(k^2,k^i)//every 256 = k^2 elements
//	 */
//
////	fd->paddingMethod
////	cudaProfilerStart();
...
//		host_fft256_permutated_for_64k (fd);
////			twiddle_general (i,k^2)
////		host_twiddle_permutated (fd, i);
//		host_twiddle_revised_permutated (fd, i);
//
////		 		permutate_256(k^i,k^2)//every 256^i = k^(2i) elements
//		for (j = 0; j < (i / 2); j++) //L(k^2,K^i)=i/2 L(k^i,k^2)
//			host_permutation_256_general_permutated (fd, (i) * 4); //permutation L(2^(i-2),256);
////		 fft_256 on whole input vector
//		host_fft256_permutated_for_64k (fd);
////		 permutate_256(k^2,k^i)//every 256 = k^2 elements
//		for (j = 0; j < (i) / 2; j++) //L(k^2,K^i)=i/2 L(k^i,k^2)
...
//	{
//
////	twiddle(K^(e-1), K); // 2^((e-1)*4)
////		host_twiddle_permutated (fd, (e - 1) * 4);
//		host_twiddle_revised_permutated (fd, e - 1);
//
////	L(K, K^(e-1)); //(e-1) L(k^(e-1),k)
//		for (j = 0; j < e - 1; j++)
//			kernel_p3_permutation_16_permutated<<<fd->inputVectorSize/256,16,256*sizeof(usfixn64)>>>(fd->device_xVector, fd->device_parameters);
//
////	FFT_16;
//		host_fft16_general (fd);
//
////	permutate(K6, K);
//		kernel_p3_permutation_16_permutated<<<fd->inputVectorSize/256,16,256*sizeof(usfixn64)>>>(fd->device_xVector, fd->device_parameters);
//
//		//twiddle(e-1,k); //means T(k^(e-1),k)
//		//host_permutation_16_permutated(fd, e-1, e);//means L(k^(e-1),k)//transposing every k^(e-1) elements
//
////		host_fft16_permutated_for_64k(fd); //FFT(k=16) on whole input vector
//		//host_permutation_16_permutated(fd, e,e-1);//means L(k,k^(e-1))//transposing every k=16 elements
////		kernel_p3_permutation_16_permutated<<<fd->inputVectorSize/256,16,256*sizeof(usfixn64)>>>(fd->device_xVector, fd->device_parameters);
//	}
