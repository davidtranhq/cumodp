...
// #include "/data/dave/canada_UWO/thesis/FFT/repository/modpn/cumodp/include/BigPrimeFieldFFT_3/bigArithmetic_multiplication_P3.h"
...
__device__ void
device_p3_bigSub_local_mult(usfixn64 *xm, usfixn64 *ym, usfixn64 *um)
{
//	usfixn64 um[8];
	short i, pos;
	unsigned short c = 0;
...
/**********************************************/
__device__ void
device_p3_bigSub_plain_local_mult (usfixn64 * __restrict__ xm, usfixn64 * __restrict__ ym)
{
//	usfixn64 um[8];
	short i, pos;
	unsigned short c = 0;
...
device_p3_bigPrimeSub_plain_inPlace_local_mult (usfixn64 * __restrict__ xm,
...
/**********************************************/
__device__ void
device_p3_bigSubZero_3_local_mult (usfixn64 &l, usfixn64 &h, usfixn64 &carry)
{
//	short i, pos;
	unsigned short c = 0;
	usfixn64 num1;
...
/**********************************************/
__device__ __inline__ void
device_p3_bigPrimeSub_plain_ptx_v0_local_mult (usfixn64 * __restrict__ xm,
																 usfixn64 * __restrict__ ym)
{
	unsigned short c = 0;
//	short pos1;
...
/**********************************************/
__device__ void
device_p3_bigPrimeAdd_correct_local_mult (usfixn64 *xm, usfixn64 *ym, usfixn64 *um)
{
	unsigned short c = 0;
	short pos1;
	usfixn64 num1, num2;
...
/**********************************************/
__device__ __inline__ void
device_p3_bigPrimeAdd_plain_inPlace_local_mult (usfixn64 * __restrict__ xm,
	usfixn64 * __restrict__ ym)
{
	unsigned short c = 0;
...
/**********************************************/
__device__ __inline__ void
device_p3_bigPrimeAdd_plain_local_mult(usfixn64 *__restrict__ xm, usfixn64 *__restrict__ ym)
{
	unsigned short c = 0;
	short pos1;
	usfixn64 num1, num2;
...
__device__ __inline__ void
device_p3_bigPrimeAdd_plain_ptx_v0_local_mult (usfixn64 * __restrict__ xm,
	usfixn64 * __restrict__ ym)
{
//	unsigned short c = 0;
...
__device__ __inline__ void
device_p3_fft_base2_permutated_local_mult (usfixn64 * xm, usfixn64 * ym,
	const usfixn64 & permutationStride)
{
	short c = 0;
	usfixn64 num1, num2;
...
/**********************************************/
__device__ void
device_p3_oneShiftRight_local (usfixn64 * xs)
{
	usfixn64 tmp;
	short i;
	{
...
/**********************************************/
__device__ void
device_p3_oneShiftLeft_local(usfixn64 * xs)
{
	usfixn64 tmp;
	short i;
...
/**********************************************/
__device__ void
device_p3_oneShiftLeft_uvector8_local (uvector8& xs)
{
	usfixn64 tmp;
	short i;
...
/**********************************************/
__device__ void
device_p3_shiftRight_uConstArray8_local_mult(uConstArray8_align8 & x, usfixn64 & tmp)
{
//	tmp = x.i[7];
//	x.i[7] = x.i[6];
//	x.i[6] = x.i[5];
...
/**********************************************/
__device__ void
device_p3_cyclicShift_plain_local (usfixn64 *xs, short sn)
{
	short i = 0, j = 0;
	usfixn64 ts[8] =
		{ 0 };
	...
	device_p3_bigSub_local_mult(xs, ts, xs);
}

/**********************************************/
__device__ __inline__ void
device_p3_negate_2_local (usfixn64 &xs, const usfixn64 &ys, short & c)
{
	usfixn64 num1;
...
/**********************************************/
__device__ void
device_p3_oneShiftRight_uvector8_local (uvector8 &xs)
{
	usfixn64 tmp;
	short i;
...
/**********************************************/
__device__ __inline__ void
device_p3_cyclicShift_permutated_12_local (usfixn64 * xs)
{
//	if (sn <= 0 || sn > 8)
//	{
//		return;
...
//		device_p3_negate_2_local(tmpX, tmpY, c);
...
//		}
	...
	for (j = 0; j < sn; j++)
		device_p3_oneShiftRight_uvector8_local (ys);
...
//	device_oneShiftLeft(ys.i);
	...
	for (j = 0; j < sn; j++)
		device_p3_oneShiftLeft_uvector8_local (ys);
...
//	//	ts2[6] = h5;
//	//	ts2[7] = h6;
//	//	device_p3_bigPrimeAdd_correct_local_mult(ts1, ts2, ts1);
//	//	ts2[0] = 0;
//	//	ts2[1] = 0;
//	//	ts2[2] = l2;
//	//	ts2[3] = l3;
...
//	//	ts2[6] = l6;
//	//	ts2[7] = l7;
//	////	device_p3_bigPrimeAdd_correct_local_mult(ts1, ts2, um);
//	//	device_p3_bigPrimeAdd_correct_local_mult(ts1, ts2, ts1);
//
//	//###############################################
//	ts1[0] = 0;
...
//###############################################
	...
	ts2[7] = 0;
//	device_p3_bigSub(ts1, ts2, um);
	device_p3_bigSub_local_mult(ts1, ts2, ts1);
	ts2[0] = h7;
	ts2[1] = 0;
	ts2[2] = 0;
	ts2[3] = 0;
...
//	device_p3_bigSub(ts1, ts2);
	...
	device_p3_bigSub_local_mult(lArray, ts2, lArray);
...
//	xs[7] = ts1[7];
	...
	device_p3_bigSub_local_mult(lArray, lArraySub, lArray);
...
device_p3_bigMult_plain_revised_v0 (usfixn64 * xs, usfixn64 * ys)
{
	...
	//	ts2[6] = h5;
	//	ts2[7] = h6;
	//	device_p3_bigPrimeAdd_plain_ptx_v0_local_mult(ts1, ts2, ts1);
	//	ts2[0] = 0;
	//	ts2[1] = 0;
	//	ts2[2] = l2;
	//	ts2[3] = l3;
	...
	//	ts2[6] = l6;
	//	ts2[7] = l7;
	////	device_p3_bigPrimeAdd_plain_ptx_v0_local_mult(ts1, ts2, um);
	//	device_p3_bigPrimeAdd_plain_ptx_v0_local_mult(ts1, ts2, ts1);
	...
	device_p3_bigPrimeAdd_plain_ptx_v0_local_mult (ts1, ts2);
	ts2[0] = l0;
	ts2[1] = l1;
	ts2[2] = l2;
	ts2[3] = l3;
	...
	ts2[6] = 0;
	ts2[7] = 0;
	//	device_p3_bigPrimeSub_plain_ptx_v0_local_mult(ts1, ts2, um);
	device_p3_bigPrimeSub_plain_ptx_v0_local_mult (ts1, ts2);
	ts2[0] = h7;
	ts2[1] = 0;
	ts2[2] = 0;
	ts2[3] = 0;
	...
	//	ts2[7] = c5;
	////	device_p3_bigPrimeSub_plain_ptx_v0_local_mult(ts1, ts2);
	//	device_p3_bigPrimeSub_plain_ptx_v0_local_mult(ts1, ts2, ts1);
	//
	//	ts2[0] = 0;
	//	ts2[1] = l1;
	//	ts2[2] = h1;
...
//			device_shiftRight_uvector8(yVector, offset);
		{
//			for (j = 0; j < i; j++)
			device_p3_shiftRight_uConstArray8_local_mult(yVector, offset);
		}
		offset = tid;
//	for (i = 0; i < step; i++)
//			if(i<step)
...
//	device_p3_bigSub_plain_local_mult(v0.i, v1.i);
	device_p3_bigPrimeSub_plain_inPlace_local_mult (v0.i, v1.i);
...
//	device_p3_bigPrimeAdd_plain(v0.i, v1.i);
	device_p3_bigPrimeAdd_plain_inPlace_local_mult (v0.i, v1.i);
...
//	device_p3_bigSub_plain_local_mult(v0_sub.i, v1.i);
//	device_p3_bigPrimeSub_plain_inPlace_local_mult(v0_sub.i, v1.i);
...
//	device_p3_bigPrimeAdd_plain(v0_sub.i, v1.i);
	device_p3_bigPrimeAdd_plain_inPlace_local_mult (v0_sub.i, v1.i);
...
//#################################################
//	device_p3_bigSub_plain_local_mult(v0.i, v0_sub.i);
	device_p3_bigPrimeSub_plain_inPlace_local_mult (v0.i, v0_sub.i);
//#################################################
...
//	printf("ps before final %lu, %lu, %lu \n",p0,p1,p2);
	...
	a1[1] = p2;
	a2[0] = t1q;
	device_p3_bigPrimeAdd_plain_local_mult(a1, a2);
	p1 = a1[0];
	p2 = a1[1];
...
//	p2 = p2 + tmp;
	...
	a1[1] = p2;
	a2[0] = t0q;
	device_p3_bigSub_plain_local_mult (a1, a2);
	p1 = a1[0];
	p2 = a1[1];
...
//			printf("x[i]=%lu, y[i]=%lu\n",x[i],y[i]);
		...
		for (i = 0; i < 8; i++)
		{
			...
			uSub[2] = 0;
			if (i > 0)
				device_p3_oneShiftRight_local (y);
			for (j = 8 - i; j < 8; j++)
			{
//				m=xs[j]*y[8-j];
				asm("{\n\t"
...
//				       cVector[6],cVector[7]);
		device_p3_oneShiftRight_local (hVectorSub);
		device_p3_oneShiftRight_local (cVectorSub);
		device_p3_oneShiftRight_local (cVectorSub);
		for (i = 0; i < 8; i++)
		{
			v0[i] = 0;
		...
		cVectorSub[0] = 0;
		cVectorSub[1] = 0;
		device_p3_bigPrimeAdd_plain_local_mult(lVectorSub, hVectorSub);

		device_p3_bigPrimeAdd_plain_local_mult(lVectorSub, cVectorSub);
		device_p3_bigSub_plain_local_mult (lVectorSub, v0);
//		printf("l \n %lu \n %lu \n %lu \n %lu \n %lu \n %lu \n %lu \n %lu \n",
//						       lVectorSub[0],lVectorSub[1],lVectorSub[2],
//						       lVectorSub[3],lVectorSub[4],lVectorSub[5],
//						       lVectorSub[6],lVectorSub[7]);
		device_p3_bigSub_plain_local_mult (lVectorSub, v1);
...
//						       lVectorSub[6],lVectorSub[7]);
		...
		device_p3_oneShiftRight_local (hVector);
		device_p3_oneShiftRight_local (cVector);
		device_p3_oneShiftRight_local (cVector);
		for (i = 0; i < 8; i++)
		{
			v0[i] = 0;
		...
		cVector[0] = 0;
		cVector[1] = 0;
		device_p3_bigPrimeAdd_plain_local_mult(lVector, hVector);
		device_p3_bigPrimeAdd_plain_local_mult(lVector, cVector);
		device_p3_bigSub_plain_local_mult (lVector, v0);
		device_p3_bigSub_plain_local_mult (lVector, v1);

		device_p3_bigSub_plain_local_mult (lVector, lVectorSub);
		offset = tid;
		for (i = 0; i < 8; i++)
		{
			xs[offset] = lVector[i];
...
//			printf ("device_p3_sub192=  %lu, %lu, %lu , sign=%lu \n", u[0], u[1], u[2], sign);
			...
			if (sign == 1)
			{
//				device_p3_bigSubZero_3_local_mult (l, h, c);
				device_p3_bigSub_plain_local_mult (v1, v0);
				l = v1[0];
				h = v1[1];
				c = v1[2];
			}
...
//				       cVector[6],cVector[7]);
		...
		{
			if (i > 0)
				device_p3_cyclicShift_plain_local (complement, 1);
			if (signVector[i] == 1)
			{
				device_p3_bigPrimeAdd_plain_local_mult(lVector, complement);
			}
		}
...
//		}
		...
		for (i = 0; i < 8; i++)
		{
			...
			if (i > 0)
//				device_p3_oneShiftRight_local (y);
				device_p3_oneShiftLeft_local(y);
...
//		v1.i[i] = hVector[tid + (i - 1) * permutationStride];
	...
	device_p3_bigPrimeAdd_plain_local_mult(v0.i, v1.i);
...
//		v0_sub.i[i] = hVectorSub[tid + (6+i) * permutationStride];
	...
	device_p3_bigPrimeAdd_plain_local_mult(v0_sub.i, v1.i);
...
//			device_3_cyclicShift_plain (complement, 1);
				device_p3_cyclicShift_permutated_12_local (complement);
			if (signVector[offset] == 1)
			{
				device_p3_bigPrimeAdd_plain_inPlace_local_mult (v0.i, complement);
//			device_p3_bigPrimeAdd_plain_ptx_v0_local_mult(v0.i,complement);
			}
			offset += permutationStride;
		}
...
//			c = 0;
			...
			if (k > 0)
			{
				device_p3_oneShiftRight_local (y.i);
			}
...
//			memset(&lhcSub[3], 0x00, 5 * sizeof(usfixn64));
			memset (lhc, 0x00, 8 * sizeof(usfixn64));
//			device_p3_bigSub_plain_local_mult(lhc, lhcSub);
			for (short x = 3; x < 8; x++)
			{
				lhc[x] = 0;
			}
...
//			device_p3_bigPrimeAdd_correct_local_mult(lhc, lhcSub,lhc);
//			device_smallSub3_plain(lhc, lhcSub, c);
...
//	device_p3_bigSub_plain_local_mult(v1.i, v0.i);
...
//	device_p3_bigSub_plain_local_mult(v1.i, v0.i);
	device_p3_bigPrimeAdd_plain_local_mult(v2.i, v0.i);
...
//		v0.i[i] = hVector[tid + (6+i) * permutationStride];
	...
	device_p3_bigPrimeAdd_plain_local_mult(v1.i, v0.i);
...
//	device_p3_bigSub_plain_local_mult(v1.i, v2.i);
	//#################################################
	offset = tid;
//	############################# writing back to g-memory
//#pragma unroll 8
