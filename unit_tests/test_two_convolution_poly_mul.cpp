...
/*
TEST(TwoConvolutionPolyMul, BigTwoConvolutionPolyMul)
{
    std::ifstream test_file("test.txt");
    if (!test_file.is_open()) {
        std::cerr << "Error opening test.txt: is it present in the directory?\n";
        FAIL();
    }

    std::string line;
    std::vector<UnivariateMPZPolynomial> polys(3);
    for (int i = 0; i < 3; ++i) {
        std::getline(test_file, line);
        std::istringstream line_stream(line);
        std::string coef;
        while (line_stream >> coef)
            polys[i].push_back(mpz_class(coef));
    }

    EXPECT_EQ(two_convolution_poly_mul(polys[0], polys[1]), polys[2]);
}
*/